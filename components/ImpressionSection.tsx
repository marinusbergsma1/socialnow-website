...
const ImpressionSection: React.FC<ImpressionSectionProps> = ({ onOpenBooking, onOpenProject }) => {
  ...
  // All scroll-driven values are set as CSS custom properties — no React re-renders
  const scrollCallback = useCallback((rect: DOMRect, viewportHeight: number) => {
    const el = stickyRef.current;
    if (!el) return;
    const totalDist = rect.height - viewportHeight;
    if (totalDist <= 0) return;
    const progress = Math.min(Math.max(Math.abs(rect.top) / totalDist, 0), 1);
    ...
    const realityScale = 1 + progress;

    el.style.setProperty('--clip-radius', `${clipRadius}%`);
    el.style.setProperty('--reality-opacity', String(realityOpacity));
    el.style.setProperty('--reality-scale', String(realityScale));
    el.style.setProperty('--impression-opacity', String(impressionOpacity > 0 && galleryOpacity < 0.5 ? 1 : 0));
    el.style.setProperty('--gallery-opacity', String(galleryOpacity));
    el.style.setProperty('--gallery-translate', `${galleryTranslate}px`);
    el.style.setProperty('--cta-opacity', String(ctaOpacity));
    el.style.setProperty('--cta-translate', `${ctaTranslate}px`);
    el.style.setProperty('--progress', String(progress));

    // Only toggle gallery mount when crossing threshold (minimises re-renders)
    const shouldShow = progress > 0.3;
    if (shouldShow !== showGallery) setShowGallery(shouldShow);
  }, [showGallery]);
  useScrollProgress(sectionRef, scrollCallback);
  ...
  const particles = useMemo(() => {
    return [...Array(50)].map((_, i) => {
        ...
        const color = ['#F62961', '#F7E644', '#00A3E0', '#ffffff'][Math.floor(Math.random() * 4)];
  ...
  return (
    <section ref={sectionRef} className="relative h-[350vh] bg-black">
      <div ref={stickyRef} className="sticky top-0 h-screen w-full overflow-hidden flex flex-col justify-center">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center transform transition-all duration-300 ease-out z-20 pointer-events-none" style={{ opacity: 'var(--reality-opacity, 1)', transform: `translate(-50%, -50%) scale(var(--reality-scale, 1))` }}>
        ...
        <div className="absolute top-[20%] left-1/2 -translate-x-1/2 text-center transform transition-all duration-300 ease-out z-20 pointer-events-none" style={{ opacity: 'var(--impression-opacity, 0)' }}>
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-6xl z-30 transition-all duration-700 ease-out" style={{ opacity: 'var(--gallery-opacity, 0)', transform: `translate(-50%, calc(-50% + var(--gallery-translate, 100px)))` }}>
            {showGallery && (
        ...
        <div className="absolute bottom-12 md:bottom-20 pointer-events-auto transition-all duration-700 transform w-[90%] md:w-auto z-50 left-1/2 -translate-x-1/2 perspective-[1000px]" style={{ opacity: 'var(--cta-opacity, 0)', transform: `translate(-50%, var(--cta-translate, 50px))` }}>
        ...
        <div className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 h-32 w-1 bg-white/10 rounded-full z-30 overflow-hidden hidden md:block">
            <div className="w-full bg-[#F7E644] rounded-full shadow-[0_0_10px_#F7E644]" style={{ height: `calc(var(--progress, 0) * 100%)` }}></div>
        </div>
      </div>
    </section>
  );
